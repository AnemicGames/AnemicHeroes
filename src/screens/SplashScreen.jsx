    ...
    return (
        <div className="relative w-full h-full overflow-hidden bg-gray-900 text-white">
            ...
            <div className="flex items-center justify-center w-full h-full relative z-10">
                <div
                    className={`${styles["fade-in-container"]} flex flex-col items-center justify-center w-fit h-full p-4`}
                >
                    <img
                        src="assets/splashScreenTitle.png"
                        alt=""
                        className={`${styles["title-descend"]} w-[450px]`}
                    />
                    <div
                        className={`${styles["fade-in-buttons"]} flex flex-col items-center space-y-2`}
                    >
                        <button
                            className={`flex px-6 py-3 rounded-full relative p-5 font-bold text-xl pl-12 border-2 ${
                                playerLevel > 0
                                    ? "bg-red-700 hover:bg-red-600 border-yellow-300"
                                    : "bg-gray-700 border-gray-300"
                            }`}
                            onClick={handleContinue}
                            disabled={playerLevel === 0}
                            onMouseEnter={() =>
                                playerLevel > 0 && setIsHovered(true)
                            }
                            onMouseLeave={() => setIsHovered(false)}
                        >
                            <img
                                src={
                                    isHovered && playerLevel > 0
